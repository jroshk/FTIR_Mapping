...
def gaussian_least_squares(df, col, peaks=yang_h20_2015,
    ...
    heights = guess_heights(df, col, peaks['means'], gain=0.95)
    width = peak_width
    lb = list()
    ub = list()
    guess = list()
...
def guess_heights(df, col, center_list, gain=0.95):
    ...
    Default of 0.95 seems to work best for most spectra, but can be changed to
    improve convergence.
...
def create_fit_plots(raw_df, col, peak_list):
    ...
    xdata = raw_df.iloc[:,0]
    y_fit = sum(peak_list)
    ax.plot(xdata, raw_df[col], label='Baseline Corrected Spectrum') #Jeff changed this to Original from '$2^{nd}$ derivative' on 3.23.2022
...
def create_fit_singleplot(raw_df, col, peak_list): # attempted to make a single plot function without residuals 5.26.22
